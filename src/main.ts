...
export interface ReadConfigRequest {
  key: string
  projectDir: string
  packageMetadata: { [key: string]: any } | null

  log?: (message: string) => void
}

export async function loadConfig<T>(request: ReadConfigRequest): Promise<T | null> {
  const data = getConfigFromPackageData(request.packageMetadata || (await orNullIfFileNotExist(readJson(path.join(request.projectDir, "package.json")))), request.key)
